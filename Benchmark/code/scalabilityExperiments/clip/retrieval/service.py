...
FAISS_INDEX_PATH = f"{CLIP_DIR}/index/clip_image_flat.index"
# FAISS_INDEX_PATH = f"{CLIP_DIR}/index/clip_image_hnsw.index"

FAISS_NAMES_PATH = f"{CLIP_DIR}/index/image_names.json"
...
class ClipFaissSemanticSearcher:
    """
    CLIP Text → Image FAISS Searcher
...
if not ok:
    raise RuntimeError("CLIP FAISS initialize() failed (check logs above)")
...
# ---------------- endpoints ----------------
@app.get("/clip_faiss_search")
async def clip_faiss_search(q: Union[str, None] = None, k: int = 10):
    if not q:
