...
def query_service(url: str, query: str, k: int, nprobe: int):
    r = requests.get(
        url,
        params={"q": query, "k": k, "nprobe": nprobe},
        proxies=PROXIES,
        timeout=120
    )
    r.raise_for_status()
...
def main():
    ...
    ap.add_argument("--service_url", default="http://127.0.0.1:8006/flava_faiss_ivfpq_search")
    ap.add_argument("--out", default="laion_flava_ivfpq_metadata_10k.json")
    ...
    for cap in tqdm(captions):
        ...
        try:
            ...
            results = query_service(args.service_url, cap, args.k, args.nprobe)
