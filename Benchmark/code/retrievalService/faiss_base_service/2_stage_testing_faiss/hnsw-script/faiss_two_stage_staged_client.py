...
def main():
    ap = argparse.ArgumentParser("Run FAISS 2-stage in two calls: stage1_only → stage2_from_file")
    ...
    ap.add_argument("--stage1_engine", default="faiss", choices=["faiss","faiss-hnsw"])
    ap.add_argument("--stage2_engine", default="faiss", choices=["faiss","faiss-hnsw"])
    ap.add_argument("--ef", type=int, default=None, help="efSearch for HNSW (optional)")
    ...
    for idx, item in enumerate(tqdm(data, desc="Two-stage (staged)")):
        ...
        s2_body = {
            ...
            "stage2_engine": args.stage2_engine,
