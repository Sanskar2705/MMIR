...
"""
FAISS Two-Stage Retrieval — Orchestrator + CLI
...
- FAISS index files created by your builder script:
    <model>_<dataset>_{faiss|faiss-hnsw}_img.index               + *_img_names.json
    <model>_<dataset>_{faiss|faiss-hnsw}_txt.index               + *_txt_meta.json
    <model>_<dataset>_{faiss|faiss-hnsw}_joint-image-text.index  + *_joint-image-text_meta.json
...
def _get_searcher(model: str, dataset: str, target: str, engine_tag: str, ef_search: Optional[int] = None):
...
class TwoStageFAISSOrchestrator:
    """
    Mirrors your Solr TwoStageRetrievalOrchestrator API, but uses FAISS.
    - load_embedding_model(model) => handled via get_embedder
    - encode_text(model, text) -> {"embedding": np.ndarray, "encoding_time": float}
    - execute_two_stage_pipeline(...) -> Dict
    - print_results(result) -> stdout
    """
    def __init__(self, ef_search: Optional[int] = None):
        ...
        print(f"FAISS Orchestrator init. Device: {_device()}  |  efSearch={self.ef_search}")
    ...
    def _search_full(self, model: str, dataset: str, target: str, engine_tag: str, q: np.ndarray, k: int):
        index, meta, _ = _get_searcher(model, dataset, target, engine_tag, self.ef_search)
        t1 = get_time()
    ...
    def _score_subset(self, model, dataset, target, engine_tag, q, image_names, k):
        index, meta, ids_by_image = _get_searcher(model, dataset, target, engine_tag, self.ef_search)
        dim = index.d
    ...
    def execute_two_stage_pipeline(
        ...
        query_text: str,
        dataset: str,
        stage1_config: Tuple[str, str, str],  # (model_name, core_type, engine_tag['faiss'|'faiss-hnsw'])
        stage2_config: Tuple[str, str, str],  # (model_name, core_type, engine_tag)
        stage1_k: int = 100,
        stage2_k: int = 10,
        ...
        s2_model, s2_core_type, s2_engine = stage2_config
        t2 = _target_from_core_type(s2_core_type)
        s2_engine_tag = _engine_tag(s2_engine)
...
def main():
    ...
    engines = ["faiss", "faiss-hnsw"]  # choose 'faiss' if unsure
